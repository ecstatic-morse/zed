...
use settings::{SettingsStore, VimSettingsContent};
...
use crate::{
    PushBeamJumpBackward, PushBeamJumpForward, PushSneak, PushSneakBackward,
    beam_jump::{BeamJumpAction, BeamJumpState},
    insert::NormalBefore,
    motion,
    state::Mode,
};
...
async fn test_sneak(cx: &mut gpui::TestAppContext) {
    ...
    cx.update(|_window, cx| {
        cx.bind_keys([
            ...
            KeyBinding::new(
                "shift-s",
                PushBeamJumpBackward { first_char: None },
...
async fn test_beam_jump_labels_forward(cx: &mut gpui::TestAppContext) {
    ...
    cx.update(|_window, cx| {
        cx.bind_keys([KeyBinding::new(
            "s",
            PushBeamJumpForward { first_char: None },
            Some("vim_mode == normal"),
        )])
    });
...
async fn test_beam_jump_labels_avoid_suffix_chars(cx: &mut gpui::TestAppContext) {
    ...
    cx.set_state("ˇ sea sef", Mode::Normal);
    cx.simulate_keystrokes("s s e");
    cx.assert_state("ˇ sea sef", Mode::Normal);
    ...
    cx.update(|_window, cx| {
        cx.update_global(|store: &mut SettingsStore, cx| {
            store.update_user_settings(cx, |s| {
                s.vim.get_or_insert_default().beam_jump_quick_sneak = Some(true);
            });
        });
...
#[perf]
#[gpui::test]
async fn test_beam_jump_comma_jumps_opposite_direction_and_exits(cx: &mut gpui::TestAppContext) {
    let mut cx = VimTestContext::new(cx, true).await;
    ...
    cx.simulate_keystrokes("S a b");
    cx.assert_state("abxxabxxabˇxxab", Mode::Normal);
...
async fn test_beam_jump_backfills_labels_after_narrowing(cx: &mut gpui::TestAppContext) {
    ...
    let text = format!("ˇx{}{}", "ab".repeat(870), "abc".repeat(10));
    cx.set_state(&text, Mode::Normal);
...
#[perf]
#[gpui::test]
async fn test_beam_jump_v0_fallback_returns_jump(cx: &mut gpui::TestAppContext) {
    let mut cx = VimTestContext::new(cx, true).await;
    ...
    let action = cx.update_editor(|editor, window, cx| {
        ...
        let mut state = BeamJumpState::new(
            Some(Direction::Next),
            false,
            "fjdkslarueiwovncmx".chars().collect::<Vec<_>>().into(),
            MultiBufferOffset(0),
    ...
    match action {
        BeamJumpAction::Jump(jump) => {
            assert_eq!(jump.direction, Direction::Next);
            assert_eq!(jump.pattern, "ab");
