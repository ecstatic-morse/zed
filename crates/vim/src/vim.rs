...
use motion::Motion;
use multi_buffer::{MultiBufferSnapshot, ToPoint as _};
...
impl Vim {
    ...
    fn activate(editor: &mut Editor, window: &mut Window, cx: &mut Context<Editor>) {
        ...
        vim.update(cx, |_, cx| {
            ...
            Vim::action(
                editor,
                cx,
                |vim, action: &PushBeamJumpForward, window, cx| {
                    vim.start_beam_jump(Some(Direction::Next), action.first_char, window, cx);
                    vim.push_operator(
                        Operator::Sneak {
                            first_char: action.first_char,
                        },
            ...
            Vim::action(
                editor,
                cx,
                |vim, action: &PushBeamJumpBackward, window, cx| {
                    vim.start_beam_jump(Some(Direction::Prev), action.first_char, window, cx);
                    vim.push_operator(
                        Operator::SneakBackward {
                            first_char: action.first_char,
                        },
            ...
            Vim::action(editor, cx, |vim, action: &PushBeamJumpBidi, window, cx| {
                vim.start_beam_jump(None, action.first_char, window, cx);
                vim.push_operator(
                    Operator::Sneak {
                        first_char: action.first_char,
                    },
    ...
    fn observe_keystrokes(
        ...
        if self.beam_jump.is_some() && keystroke_event.keystroke.key == "backspace" {
    ...
    fn cancel_beam_jump(&mut self, cx: &mut Context<Self>) {
        if let Some(state) = self.beam_jump.as_mut() {
            Vim::globals(cx).last_find = state.previous_last_find.take();
        }
    ...
    fn clear_beam_jump(&mut self, cx: &mut Context<Self>) {
        ...
        self.beam_jump = None;
        self.update_editor(cx, |_, editor, cx| {
            editor.clear_beam_jump_highlights(cx);
        });
    }

    fn compute_visible_range(
        buffer: &MultiBufferSnapshot,
        layout: &movement::TextLayoutDetails,
    ) -> (MultiBufferOffset, MultiBufferOffset) {
        if let Some(visible_rows) = layout.visible_rows {
    ...
    fn start_beam_jump(
        ...
        let VimSettings {
            use_smartcase_find: smartcase,
            ref beam_jump_chars,
            ..
        } = *VimSettings::get_global(cx);
        let beam_jump_chars = beam_jump_chars.clone();
        ...
        self.update_editor(cx, |vim, editor, cx| {
            ...
            let jump_start = match direction {
                Some(Direction::Next) | None => right_of_cursor,
                Some(Direction::Prev) => cursor_offset,
            };
        ...
        if let Some(first_char) = first_char {
            self.beam_jump_input(first_char.to_string().into(), window, cx);
        }
    }

    fn beam_jump_input(&mut self, text: Arc<str>, window: &mut Window, cx: &mut Context<Self>) {
        ...
        let action = self
            .update_editor(cx, |vim, editor, cx| {
                let Some(state) = vim.beam_jump.as_mut() else {
                    return BeamJumpAction::Cancel;
                };

                if state.num_pattern_chars() >= 2 && matches!(ch, ';' | ',') {
                    let quick_sneak = VimSettings::get_global(cx).beam_jump_quick_sneak;
                    if quick_sneak {
                        let direction = state.direction.unwrap_or_default();
                ...
                if matches!(action, BeamJumpAction::Continue) {
                    let highlights = state
                        .candidates_and_labels()
                        .map(|(m, label)| editor::BeamJumpHighlight {
                            range: m,
                            label: label.cloned(),
                        })
                        .collect::<Vec<_>>();
                    editor.set_beam_jump_highlights(highlights, cx);
                }
    ...
    fn input_ignored(&mut self, text: Arc<str>, window: &mut Window, cx: &mut Context<Self>) {
        ...
        match self.active_operator() {
            ...
            Some(Operator::Sneak { .. } | Operator::SneakBackward { .. })
                if self.beam_jump.is_some() =>
            {
                self.beam_jump_input(text, window, cx);
            }
...
struct VimSettings {
    ...
    pub beam_jump_chars: Arc<[char]>,
    pub beam_jump_quick_sneak: bool,
...
impl Settings for VimSettings {
    fn from_settings(content: &settings::SettingsContent) -> Self {
        ...
        Self {
            ...
            beam_jump_quick_sneak: vim.beam_jump_quick_sneak.unwrap(),
            beam_jump_chars: vim
                .beam_jump_chars
                .unwrap()
                .chars()
